...
void tower(int number, int src, int dest, int temp)
{
	if (number == 0)
		return;
		tower(number - 1, src, temp, dest );
	cout <<"C " <<src << "го стержня кладем диск на  " << dest << "-й"<<endl;
		tower(number - 1, temp, dest, src );
}
...
void main()
{
	...
	do {
		system("CLS");
		cout << " Введите количество дисков от 1 до 64:   ";
		cin >> number;
	} while (number <= 0 || number > 64);
